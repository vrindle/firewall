...
import unittest
try:
    from unittest.mock import call, DEFAULT, MagicMock, Mock, patch
except ImportError:
    from mock import call, DEFAULT, MagicMock, Mock, patch
...
class FirewallLibMain(unittest.TestCase):
    ...
    def test_permanent_runtime_offline(self, am_class):
        ...
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="One of permanent, runtime or offline needs to be enabled")
    ...
    def test_timeout_with_disabled_state(self, am_class):
        ...
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="timeout can not be used with state: disabled")

    @patch("firewall_lib.AnsibleModule", new_callable=MockAnsibleModule)
    @patch("firewall_lib.HAS_FIREWALLD", True)
    def test_masquerade_with_disabled_state(self, am_class):
        am = am_class.return_value
        am.params = {"source": ["192.0.2.0/24"], "state": "disabled", "masquerade": True}
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="masquerade can not be used with state: disabled")
    ...
    def test_icmp_block_inversion_with_disabled_state(self, am_class):
        ...
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="icmp_block_inversion can not be used with state: disabled")
    ...
    def test_main_error_timeout_interface(self, am_class):
        ...
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="timeout can not be used with interface only")
    ...
    def test_firewalld_running(self, am_class, firewall_class):
        ...
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="Firewalld is not running and offline operation is declined.")
    ...
    def test_firewalld_offline_version_disconnected(self, am_class, firewall_class):
        ...
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="Unsupported firewalld version 0.3.8, offline operation requires >= 0.3.9")
    ...
    def test_firewalld_offline_version_connected(self, am_class, firewall_class):
        ...
        with self.assertRaises(MockException):
            firewall_lib.main()
        am.fail_json.assert_called_with(msg="Unsupported firewalld version 0.2.8, requires >= 0.2.11")
    ...
    def test_firewall_service_enabled_state(self, am_class, firewall_class):
        ...
        call_list = [call("default", "https", 0), call("default", "ipsec", 0), call("default", "ldaps", 0)]
    ...
    def test_firewall_port_enabled_state(self, am_class, firewall_class):
        ...
        firewall_lib.main()
        fw.setExceptionHandler.assert_called_once()
        call_list = [call("default", "8081", "tcp", 0), call("default", "161-162", "udp", 0)]
    ...
    def test_firewall_rich_rule_enabled_state(self, am_class, firewall_class):
        ...
        am.params = {
            "permanent": False,
            "offline": True,
            "richrule": ['rule protocol value="a" accept', 'rule protocol value="b" reject'],
            "state": "enabled",
            "runtime": True,
        }
        fw = firewall_class.return_value
        ...
        firewall_lib.main()
        fw.setExceptionHandler.assert_called_once()
        call_list = [call('rule protocol value="a" accept'), call('rule protocol value="b" reject')]
